...
function isFocusedSessionMessage(msg: any): boolean {
  const { sessionId, sessionPath } = sessionIdentityFromMessage(msg);
  // Studio bridge always tags stream events with sessionPath. Identity-less
  // events must NOT attach to whatever happens to be focused — that made
  // background turns paint into the session you just switched to.
  if (!sessionId && !sessionPath) return false;
