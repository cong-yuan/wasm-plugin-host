...
export function ModelSelector({ models, sessionModel, isStreaming = false }: {
  ...
  const refreshModels = useCallback(async (e?: MouseEvent) => {
    e?.preventDefault();
    e?.stopPropagation();
    ...
    try {
      ...
      useStore.getState().addToast(t('model.refreshFailed') || '刷新模型列表失败', 'error');
    } finally {
      setRefreshing(false);
    }
  }, [refreshing, t]);
  ...
  // If a refresh brings the session model back into the catalog, clear the
  // sticky "unavailable" badge without requiring a session switch.
  useEffect(() => {
    const path = useStore.getState().currentSessionPath;
    if (!path || !sessionModel?.id || !sessionModel.provider) return;
    if (sessionModel.available !== false) return;
    if (!matchedSessionModel) return;
  ...
  }, [sessionModel, matchedSessionModel]);
  ...
  return (
    <SelectWidget
      ...
      renderPopupHeader={() => (
        <div className={selectWidgetStyles.popupHeader}>
          <button
            type="button"
            className={`${selectWidgetStyles.popupHeaderBtn}${refreshing ? ` ${selectWidgetStyles.spinning}` : ''}`}
            disabled={refreshing || loading}
            onMouseDown={(e) => { e.preventDefault(); e.stopPropagation(); }}
            onClick={(e) => { void refreshModels(e); }}
            title={t('model.refreshList') || '刷新可用模型'}
          >
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
              <polyline points="23 4 23 10 17 10" /><polyline points="1 20 1 14 7 14" />
              <path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15" />
            </svg>
            <span>{refreshing ? (t('model.refreshing') || '刷新中…') : (t('model.refreshList') || '刷新')}</span>
          </button>
        </div>
      )}
    />
