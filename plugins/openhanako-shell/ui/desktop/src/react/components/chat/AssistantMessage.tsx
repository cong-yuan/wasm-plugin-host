...
export const AssistantMessage = memo(function AssistantMessage({
  ...
  const hasTextContent = blocks.some((block) => {
    if (block.type !== 'text') return false;
    const source = typeof (block as { source?: string }).source === 'string' ? (block as { source?: string }).source! : '';
    const html = typeof (block as { html?: string }).html === 'string' ? (block as { html?: string }).html! : '';
    return source.trim().length > 0 || html.trim().length > 0;
  });
  const showWaitingDots = isStreaming && !isInterludeOnly && !hasTextContent;
