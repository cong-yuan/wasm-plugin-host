...
export function ProviderModelList({ providerId, summary, onRefresh }: {
  ...
  const loadDiscoveredModels = async () => {
    try {
      ...
      if (Array.isArray(models) && models.length > 0) setAvailableCount(models.length);
    } catch {
      // cache miss is fine
    }
  };
  ...
  const fetchModels = async () => {
    ...
    try {
      ...
      setDiscoveredModels(models);
      setAvailableCount(models.length);
      setSearch('');
    ...
    <div className={styles['pv-models']}>
      ...
      <div className={styles['pv-models-action-row']}>
        ...
        >
          ...
          {fetchingModels ? t('settings.providers.fetchingModels') : t('settings.providers.fetchModels')}
          {!fetchingModels && availableCount != null && availableCount > 0 && (
            <span className={styles['pv-fetch-available']}>
              {t('settings.providers.availableCount', { n: availableCount })}
            </span>
          )}
        </button>
      </div>
