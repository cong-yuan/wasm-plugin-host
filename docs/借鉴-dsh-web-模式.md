...
| 优先级 | 项 | 成本 | 收益 |
|---|---|---|---|
| ~~高~~ | ~~双源加载防重~~ | — | ❌ **探测后判定为非真问题，未采纳**——见 [`路线图`](路线图-功能复刻.md) 第 3 节 |
| **高** | **翻译完整性类型**（`Record<keyof typeof zh, string>`） | 极小 | 编译期保证不漏翻译 |
| **高** | **`data-*` 当公开契约**（我们已在写，但未文档化） | 小 | 插件可稳定用 CSS 定位；避免无意改动破坏插件 |
| ✅ 高 | **disposer 带 label** | 小 | 已做：`PluginHandle.track(label, run)` |
| 中 | **core 共享层**（插件内宿主/前端共用逻辑） | 中 | 消灭两侧重复实现 |
| ✅ 中 | **批量挂载容错**（一处失败不中断其余） | 小 | 已做：`mount` / `mountComponent` / teardown 三处 |
