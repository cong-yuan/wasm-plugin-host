...
| [**测试.md**](测试.md) | 35 个测试(32 集成 + 3 单元)逐个说明验证了什么 | 改代码怕改坏的人 |
...
├────────────────────────────────────────────────────────────┤
│  Runtime      编译缓存 · 池化分配 · WASI · host.* 导入        │
│  Pipe/State   有界日志 + WASI stdout/stderr 捕获(fd 无关语言) │
└───────────────────┬────────────────────────────────────────┘
                    │ wasmtime 44
      ┌─────────────┼─────────────┬──────────────┐
      ▼             ▼             ▼              ▼
...
**功能完整度 ~85%**:插件生命周期、热更新、流程介入、服务图、配置、日志、并发全部完成且
有测试(32 集成 + 3 单元,全绿,零警告)。
**最大缺口**:没有能力/权限模型(仍是完整 WASI,不能安全跑第三方不可信插件),以及尚未接 Tauri。
